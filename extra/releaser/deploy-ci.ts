...
LIBS.map(libName => {
  ...
  shell.exec(
    `rsync -a --exclude 'apps' --exclude 'extra' --exclude 'ios' --exclude 'android' --exclude 'tools' --exclude 'node_modules' --exclude '.build' --exclude '.storybook' --exclude '.vscode' --exclude 'coverage' --exclude 'dist' ../../ ./.build/${libName}`
  );
  shell.exec(`cp -rf ./readme/${libName}.md ./.build/${libName}/README.md`);
  shell.exec(`cp -rf ../../libs/${libName}/.travis.yml ./.build/${libName}`);
  shell.exec(`cp -rf ../../libs/${libName}/.gitlab-ci.yml ./.build/${libName}`);
  shell.exec(
