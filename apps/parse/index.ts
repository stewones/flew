 ...
 * Configure Reative
 */
import { install } from '@reative/parse';
import { Reative, collection } from '@reative/core';

/**
 * Set default driver
 */
Reative.options.driver = `parse`;
...
app.get('/', function(req, res) {
  res.status(200).send(`Reative App`);
});
...
httpServer.listen(port, function() {
  console.log('Reative App Server running on port ' + port + '.');
});
