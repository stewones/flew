(function(f){if(typeof exports==="object"&&typeof module!=="undefined"){module.exports=f()}else if(typeof define==="function"&&define.amd){define([],f)}else{var g;if(typeof window!=="undefined"){g=window}else if(typeof global!=="undefined"){g=global}else if(typeof self!=="undefined"){g=self}else{g=this}g.reative = f()}})(function(){var define,module,exports;return (function(){function r(e,n,t){function o(i,f){if(!n[i]){if(!e[i]){var c="function"==typeof require&&require;if(!f&&c)return c(i,!0);if(u)return u(i,!0);var a=new Error("Cannot find module '"+i+"'");throw a.code="MODULE_NOT_FOUND",a}var p=n[i]={exports:{}};e[i][0].call(p.exports,function(r){var n=e[i][1][r];return o(n||r)},p,p.exports,r,e,n,t)}return n[i].exports}for(var u="function"==typeof require&&require,i=0;i<t.length;i++)o(t[i]);return o}return r})()({1:[function(require,module,exports){
"use strict";
...
function find(handler) {
    ...
    var verb = handler.chain.query && handler.chain.query['aggregate']
        ? 'aggregate'
    ...
    //
    // define adapter
    var connector = new handler.Parse.Query(handler.collection);
    //
    // Transpile chain query
    var query = transpile_1.transpileChainQuery(chain.query, handler);
    //
    // Join query with connector
    if (!lodash_1.isEmpty(query)) {
        connector = (_a = handler.Parse.Query).and.apply(_a, query);
    }
    //
    ...
    //
    // set where
    where_1.where(chain.where, connector);
    //
    // set order
    order_1.order(chain.sort, connector);
    //
    // set limit
    if (chain.size)
        limit_1.limit(chain.size, connector);
    //
    // set skip
    if (chain.after)
    ...
    switch (verb) {
        case 'aggregate':
            connector
                ...
                .then(function (r) { return handler.success({ data: r }); })["catch"](handler.error);
            break;
        default:
            connector
                .find({
...
function order(sort, connector) {
    if (lodash_1.isArray(sort)) {
        // this.log().success()(`parse sort array -> ${sort}`);
        sort.map(function (s) {
            if (lodash_1.isEmpty(s))
                throw new Error("sort object in array can't be null");
            for (var k in s) {
                if (s[k] === 'asc') {
    ...
    else if (lodash_1.isObject(sort)) {
        // this.log().success()(`parse sort object -> ${JSON.stringify(sort)}`);
        if (lodash_1.isEmpty(sort))
            throw new Error("sort object can't be null");
        for (var k in sort) {
            if (sort[k] === 'asc') {
                connector.ascending(k);
            }
...
function setWhere(q, connector) {
    switch (q.operator) {
        ...
        case '<':
            ...
            break;
        case 'array-contains':
            connector.containedIn(q.field, lodash_1.isArray(q.value) ? q.value : [q.value]);
            break;
        default:
            break;
    }
...
function transpileChainQuery(query, handler) {
    ...
    for (var k in query) {
        ...
        if (handler.skipOnQuery.includes(k))
            return;
        if (handler.skipOnOperator.includes(k))
            continue;
        //
        // Tranpile query
...
exports.transpileChainQuery = transpileChainQuery;
function transpileQuery(operator, chainQuery, handler) {
    //
    // Hold queries
    var queries = [];
    //
    ...
    if (handler.specialOperators.includes(operator)) {
        ...
        chainQuery = lodash_1.isArray(chainQuery) ? chainQuery : [chainQuery];
        //
    ...
    // Transpile common operators
        ...
        //
        // Get operator value
        var value = chainQuery[operator] ? chainQuery[operator] : chainQuery;
        //
        // Treatment for arrays
        if (lodash_1.isArray(value) && !lodash_1.isString(value[0])) {
...
exports.transpileQuery = transpileQuery;
function transpileQueryRouter(specialOperator, chainQuery, handler) {
    var _a;
    //
    // Hold queries
    var queries = [];
    //
    // Transpile queries
    chainQuery.map(function (operators) {
        for (var operator in operators) {
            ...
            var nextChainQuery = handler.specialOperators.includes(operator)
                ? operators[operator]
                : operators;
            //
            // Tranpile query
    ...
    //
    // Validate
    if (lodash_1.isEmpty(queries))
        return queries;
    return (_a = handler.Parse.Query)[specialOperator].apply(_a, queries);
}
exports.transpileQueryRouter = transpileQueryRouter;
function createQueryByOperator(value, operator, handler) {
    //
    // Start query
    var query = new handler.Parse.Query(handler.collection);
    //
    // Create from a function
    if (lodash_1.isFunction(value)) {
        query[operator].apply(query, value());
    }
    else if (lodash_1.isArray(value)) {
        value.map(function (it) { return createQueryByOperator(it, operator, handler); });
    }
    else {
